...
5. **统计功能**：记录解析统计数据
6. **GIF 智能处理** 🎬：检测到多个 GIF 时自动跳过上传，提供原始下载链接（节省时间和流量）
...
#### 1. **GIF 智能处理** 🎬
自动检测多个 GIF 时跳过上传，改为提供原始下载链接。

**工作原理**：
- 检测到 **纯 GIF 内容**（所有媒体都是 GIF）
- 且 GIF 数量 **超过 5 个**
- 自动跳过下载和上传，显示下载按钮
...
- ⚡ 节省上传时间（GIF 上传慢）
- 💾 节省流量（用户按需下载）
- 🎯 保持原始质量（Telegram 会将大 GIF 转为 document）
- 🔗 一键访问所有 GIF 原始链接

**按钮样式**：
```
⚠️ 检测到 8 个 GIF，数量过多已跳过上传
💾 请点击下方按钮下载原始文件
...
**配置**（可选）：
修改 `utils/gif_helper.py` 中的阈值：
```python
GIF_ONLY_SKIP_DOWNLOAD_COUNT_THRESHOLD = 5  # 默认5个，可调整
```
