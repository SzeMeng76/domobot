...
**权限说明（满足任一条件即可）：**
1. ✅ **Super Admin** - `SUPER_ADMIN_ID` 配置的用户（已有配置）
2. ✅ **Admin** - 通过 `/admin add <user_id>` 添加的管理员
3. ✅ **白名单用户** - 通过 `/whitelist add <user_id>` 添加的用户

三者是**或**的关系，满足任一条件即可使用Guest Bot。
无需额外配置，使用现有的权限系统！
...
async def setup_handlers(application: Application, config):
    ...
    if config.enable_guest_bot:
        ...
        guest_bot_handler = GuestBotHandler(
            ...
            config=config  # 传入config获取super_admin_ids
        )
...
2. 你已经有权限（Super Admin / Admin / 白名单用户）
3. 在群组发送：`@你的bot /help`
4. Bot会在群组中回复你！
