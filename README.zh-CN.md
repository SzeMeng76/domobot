...
-   🎬 **电影和电视剧信息:** 查询电影/电视剧详情及海报、评分、演员、预告片、评价、推荐、观看平台和季集信息。支持多源数据整合（TMDB + Trakt + JustWatch），Telegraph长内容集成，热门趋势发现、人物搜索、增强的统计数据（观看数、社区数据），以及全面的流媒体平台排行榜和跨平台对比功能。*(需要白名单)*
...
# 流媒体平台排行榜和排名
/charts                    # 交互式流媒体平台排行榜中心
/chart_compare 阿凡达      # 跨平台可用性对比
...
/movie_cleancache         # 清理电影/电视和流媒体排行榜缓存
/steamcc                  # 清理Steam缓存
...
#### 流媒体平台排行榜和排名 (最新功能)
- **交互式排行榜中心:** 全面的 `/charts` 命令支持40多个流媒体平台，包括Netflix、Disney+、HBO Max、Amazon Prime、Apple TV+、Hulu、Paramount+等
- **跨平台对比功能:** 动态 `/chart_compare` 功能，使用JustWatch实时数据比较多个流媒体服务的内容可用性
- **平台专属排行榜:** 针对每个流媒体平台的专用排行榜，显示新上架内容和热门内容，优化平台发现体验
- **层次化菜单系统:** 有序的平台分类（订阅、免费、租赁），支持智能内容过滤和基于质量的排序
- **区域内容发现:** 特定国家的热门内容，支持美国、英国、日本、韩国、法国、德国、加拿大、澳大利亚和巴西
- **内容分类:** 按流派（动作、喜剧、恐怖、爱情、科幻）和表现指标（前10、前100、上升趋势、新入榜、长期热门）的高级过滤
- **动态内容选择:** AI驱动的跨平台对比内容策划，确保高质量主流内容具有多平台可用性
- **全面缓存集成:** 与现有缓存管理系统完全集成，通过 `/movie_cleancache` 实现最佳性能
...
#### 电影电视功能
- **多源数据整合:** 结合TMDB、Trakt和JustWatch API提供全面的电影/电视剧信息
- **增强搜索:** 交互式按钮电影/电视剧选择
- **丰富详情:** 海报、评分、演员、剧组、预告片和观看平台
...
- **热门内容:** 专门的热门和流行电影/电视剧命令
- **季集信息:** 详细的电视剧分集信息，智能内容截断
- **人物搜索:** 演员、导演和剧组信息及作品履历
- **流媒体平台排行榜:** 交互式排行榜中心，支持40多个流媒体平台，包括Netflix、Disney+、HBO Max、Amazon Prime、Apple TV+、Hulu等
- **跨平台对比:** 多个流媒体服务间的动态内容可用性对比，提供实时数据
- **平台专属排名:** 每个流媒体平台的专用排名，显示新上架和热门内容
- **区域内容发现:** 特定国家的热门内容和平台可用性（按地区）
- **内容分类:** 按流派（动作、喜剧、恐怖等）和表现指标（前10、上升趋势、新入榜）的排名
- **数据源透明:** 所有显示中清晰标注数据来源（🎬 TMDB，📺 Trakt）
...
- **JustWatch API:** 用于流媒体平台排行榜、跨平台可用性和平台专属排名
- **和风天气API:** 用于天气预报数据
- **Steam API:** 用于游戏价格信息
- **各种流媒体服务API:** 用于订阅价格查询
