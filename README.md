...
-   🔴 **Reddit Post Parser:** Whitelist required - comprehensive Reddit post parsing and listing functionality. Features **OAuth 2.0 authentication** (client credentials flow), **single post parsing** with support for images, videos, and galleries upload, **hot posts listing** (`/reddit hot [subreddit]`), **top posts listing** (`/reddit top [subreddit] [time]` supporting hour/day/week/month/year/all), **AI Chinese translation** powered by OpenAI API for content summaries and list translations, **large video upload** with Pyrogram priority supporting up to 2GB, **inline mode** for direct Reddit link input or `reddit hot/top` commands, **interactive buttons** for original post links and AI summary/translation, **Redis caching** with 24-hour TTL, **weekly auto-cleanup**, and **MarkdownV2 formatting**. Commands: `/reddit <link>`, `/reddit hot [subreddit]`, `/reddit top [subreddit] [time]`. *(Whitelist required)*
...
- **NetEase Cloud Music API:** For song search, download, lyrics, and metadata via EAPI encryption
