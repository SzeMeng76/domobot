            ...
            if actual_index < len(search_data["all_results"]):
                ...
                if item_id:
                    ...
                    try:
                        ...
                        if success:
                            logger.info(f"✅ 调度删除Steam价格详情: chat_id={(query.message.chat_id if query.message else None)}, message_id={(query.message.message_id if query.message else None)}, delay={config.auto_delete_delay}秒")
                        else:
