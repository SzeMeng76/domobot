...
async def get_netflix_prices_text(cache_manager_ref, rate_converter_ref) -> str:
    """
    获取 Netflix 价格文本（供 inline_command_adapter 调用）

    Args:
        cache_manager_ref: 缓存管理器
        rate_converter_ref: 汇率转换器

    Returns:
        格式化的价格文本
    """
    if not netflix_price_bot:
        return "❌ Netflix 查询服务未初始化"

    try:
        await netflix_price_bot.load_or_fetch_data(None)
        return await netflix_price_bot.get_top_cheapest()
    except Exception as e:
        logger.error(f"获取 Netflix 价格失败: {e}")
        return f"❌ 获取 Netflix 价格失败: {str(e)}"
