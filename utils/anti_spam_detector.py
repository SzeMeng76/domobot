...
3. 非法支付、赌博、禁止物品贩卖
4. **主动推销非法服务**：飞机会员、刷单、赌台、网赚、日入千金等（注意：区分"推销"和"询问"，用户询问"有人卖XX吗？"、"需要XX，有卖的吗？"是正常提问，不是广告）
...
- **区分询问和推销**：用户询问"需要XX，有卖的吗？"、"有人卖XX吗？"、"哪里可以买到XX？"是正常的需求提问，不是广告推销。只有主动推销"出售XX"、"代办XX"、"提供XX服务"才是广告
- **新用户的正常提问不应误封**：即使是0天0发言的新用户，如果只是询问购买某个服务/产品，这是正常的群组互动，不应判定为垃圾广告。只有当新用户主动推销、引流、发送明显广告内容时才封禁
