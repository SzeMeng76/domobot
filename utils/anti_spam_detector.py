...
- 风险评分：{risk_score}/100"""
        ...
        if bio:
            ...
            user_profile += "\n     - 自我介绍：描述用户是谁、做什么的、有什么爱好/职业、个人联系方式、社交媒体/项目链接。"
            user_profile += "\n       例：'技术宅 | Web开发者 | https://twitter.com/xxx' → 自我介绍"
            user_profile += "\n       例：'PM Bot | 私聊机器人 | t.me/MyBot' → 自我介绍（个人联系方式）"
            user_profile += "\n       例：'联系我请用：@myusername' → 自我介绍（个人联系方式）"
            user_profile += "\n     - 广告/引流：向陌生人推销服务/项目/收益机会，附加联系方式或入群链接，包含商业推广语义。"
            user_profile += "\n       例：'进群了解，自己在平台洗金 t.me/xxx' → 广告/引流"
            user_profile += "\n       例：'今年最新项目一天稳定8万来找我小白也带，进群链接：t.me/xxx' → 广告/引流"
            user_profile += "\n       例：'承接各类支付通道 USDT 兑换 联系 @xxx' → 广告/引流"
            user_profile += "\n       例：'私聊机器人代理 | 日入500 | 联系 @xxx' → 广告/引流（有收益承诺）"
            user_profile += "\n"
            user_profile += "\n  ❷ 关键：如何区分「个人联系方式」vs「商业引流广告」？"
            user_profile += "\n     ✅ 个人联系方式（正常）："
            user_profile += "\n        • 单纯提供联系方式：'私聊请用 @xxx'、'Email: xxx'、'PM Bot: t.me/xxx'"
            user_profile += "\n        • 个人项目/作品展示：'我的博客'、'GitHub'、'开源项目'"
            user_profile += "\n        • 社交媒体账号：Twitter、Instagram、频道链接"
            user_profile += "\n        • 判断标准：没有商业推广语义（无收益承诺、无付费服务、无招募代理）"
            user_profile += "\n     ❌ 商业引流广告（高风险）："
            user_profile += "\n        • 包含收益承诺：'日入X万'、'稳定收益'、'带你赚'、'保底'"
            user_profile += "\n        • 付费服务推广：'代理'、'批发'、'出售'、'承接业务'"
            user_profile += "\n        • 召唤性引流：'进群了解'、'看简介赚钱'、'加我领红包'、'扫码入群'"
            user_profile += "\n"
            ...
            user_profile += "\n     ⚡ 真正的「自我介绍」不会向陌生人推销服务、不会承诺收益、不会说'进群了解赚钱'。"
            user_profile += "\n     ⚡ 但单纯的联系方式（'私聊用bot'、'@username'、'Email'）是正常的，不算引流。"
            user_profile += "\n"
...
10. 分享联系人广告（分享联系人消息，联系人姓名包含广告内容：赌博、网赚、货币兑换、支付通道、联系方式、Telegram链接等。这是常见的绕过文本检测的广告手段，评分>=85）
11. @广告bot召唤（消息中@mention某个bot，同时配合赌博/洗钱/网赚/引流语义词，例如"@xxxbot 洗米来"、"@xxxbot 带你赚"、"@xxxbot 进群"等。这是用bot代替人工回复广告的手法，评分>=85）
12. 色情/低俗骚扰@人（消息含明显色情、性暗示、低俗侮辱性词语，同时 @mention 某个用户名，且该用户名与当前对话上下文无关、像是随机/批量选中的陌生人。这是常见的色情骚扰刷屏手法，目的是骚扰对方或借低俗内容博曝光，评分>=80）

判定规则：
- 推销vs询问：主动"出售XX"是广告，询问"有人卖XX吗"是正常
- 简介链接不是依据：个人简介中的链接（个人联系方式/社交媒体/个人项目）是正常的，除非消息本身包含主动推销
- 货币兑换：主动提供exchange是广告，询问"有人能换汇吗"是正常
- 订阅服务：主动推广subscription/VIP/premium，评分>=85
- 昵称含货币关键词（USDT/BTC/NAIRA/EXCHANGE）的用户发推广消息，+25分
- 🚨 简介高风险 → 自动判定养号顶帖：严格按上方「简介风险评估」五步流程推理。先判断简介是「自我介绍」还是「广告/引流」，注意区分个人联系方式（正常）vs 商业引流（广告）。一旦判定简介属于高风险广告/引流（赌博/跑分/洗钱、网赚、非法支付、加密喊单、灰产、商业引流话术任一），无论当前消息是什么（'.'、'你好'、'1'、表情、单字、贴纸都算），都判定为养号顶帖：state=1, spam_score>=85。关键：单纯的联系方式（'私聊用bot'、'@username'、'Email'）不算引流，但配合收益承诺/付费服务的就是广告。
- 🚨 引流看简介：消息含"看简介"、"点简介"、"看号主"、"点击此号"等引导查看简介的话术，且简介含推广内容，直接判定垃圾广告，评分>=85
- 🚨 色情/低俗骚扰@人：消息含色情/性暗示/低俗侮辱性词语并 @mention 一个用户名，评分>=80。关键区分：如果被 @ 的人是群内正在互动的对象（如回复对方消息、玩梗、朋友间开荤段子），属于正常群内互动，不算骚扰；如果是消息里突然冒出一个跟上下文无关的 @用户名（很像随机挑选/批量投放），则属于骚扰式刷屏，按此条判定
- 新用户正常提问/闲聊不封，只封主动推销/引流
- 老用户（>7天且>10次发言）短消息更宽容
- ⚠️ 引用/回复消息时的判定（重要）：
  • 必须先判断被引用/回复的消息类型：
    (a) 垃圾广告：恶意推广、引流、赌博、非法服务等
    (b) 群内商业活动：竞价、拍卖、求购、团购、限时优惠等群内允许的交易活动
  • 如果是【群内商业活动】：
    - 用户回复符合活动规则的内容（如竞价出价、报名、询价）→ 正常交易行为，state=0, spam_score<30
    - 判断依据：检查被引用消息是否包含明确的活动规则、截止时间、操作指南、官方客服等合法商业要素
  • 如果是【垃圾广告】：
    - 用户消息很短（<10字）且无实质内容（如"👏"、"好"、"vk9"、"+1"）→ 判定为顶帖行为，评分>=85
    - 用户消息有实质内容或正常交流 → 视情况判定
  • 关键区分：DO账号竞价、VPS拍卖、群内团购等带有规则说明和客服联系方式的活动属于【群内商业活动】，不是垃圾广告
- 短消息豁免规则（默认情况）：短消息（<10字）且无明显广告特征，认定正常（state=0, spam_score<50）。
- ⚠️ 短消息豁免的反向条件（不豁免）：以下任一情况，短消息也不能豁免，按上方规则判定：
  (1) 引用/回复了垃圾广告且为顶帖行为（注意区分群内商业活动）
  (2) 简介判定为广告/引流（按上方四步流程，养号曝光行为）
  (3) 用户昵称含货币兑换关键词
  (4) 风险评分>=60 且简介非空
  (5) 疑似养号测试：入群0天、0发言、无头像、简介是无意义随机字符串（连续10+个字母/数字无空格无标点，如"LJvIJXhTuDqWMI6hamM"），且风险评分>=30。这类账号发任何短消息都判定为养号探测（spam_score>=70）
