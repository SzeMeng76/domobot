...
- 风险评分：{risk_score}/100"""
        ...
        if bio:
            ...
            user_profile += "\n  🚨 极高风险：如果简介包含赌博/网赚关键词（日入、稳定收益、公群担保、红包奖励等），且消息内容疑似推广（包括谐音、混淆字符、短语），直接判定为垃圾广告，评分>=85"
...
- 🚨 简介含赌博/引流关键词：如果简介包含"日入"、"稳定收益"、"公群担保"、"红包奖励"、"找人合作"、"靠谱项目"等，且消息疑似推广（包括谐音字、混淆字符、短语如"o疯险"），直接判定垃圾广告，评分>=85
...
- 短消息（<10字）且无明显广告特征，强制认定正常（state=0, spam_score<50）。但如果引用了垃圾广告或简介含赌博/引流关键词，不适用此规则
